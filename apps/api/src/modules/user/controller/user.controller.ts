...
export const listUserController: RequestHandler = async (req, res, next) => {
  try {
    ...
    const userList = await listUsers(restaurantId, currentUserId, Number(page), Number(limit));
...
export const updateUserController: RequestHandler = async (req, res, next) => {
  try {
    ...
    const user = await updateUserService(restaurantId, currentUserId, id, req.body);
...
export const deleteUserController: RequestHandler = async (req, res, next) => {
  try {
    ...
    const currentUserId = getUserId(req);
    const id = req.params.id as string;
    await deleteUserService(restaurantId, currentUserId, id);
